...
void analyzeAudioWithFFTW(const std::vector<float> &pcmDataFloat) {
    ...
    int N = pcmDataFloat.size(); // Number of samples in the PCM data
    ...
    for (int i = 0; i < N / 2 + 1; ++i) {
        ...
        if (magnitudes[i] > 0.1) // Print only significant magnitudes
            std::cout << "Magnitude at " << frequency << " Hz: " << magnitudes[i] << std::endl;
    }
                ...
                if (!msg.audioData().empty() && (tg > 0)) {
                    ...
                    while (pcmSampleBuffer.size() >= samples_size) {
                        std::vector<float> batchToProcess(pcmSampleBuffer.begin(),
                                                          pcmSampleBuffer.begin() + samples_size);
